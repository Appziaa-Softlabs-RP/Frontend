...
export const CategoryPage = () => {
    ...
    const resetSortFilter = () => {
        setProductData([]);
        setProductData(ProductActualData);
    }

    const priceAscending = () => {
        setProductData([]);
        let OriginalProducts = ProductData;
        OriginalProducts.sort((p1, p2) => (parseInt(p1.mrp) < parseInt(p2.mrp)) ? 1 : (parseInt(p1.mrp) > parseInt(p2.mrp)) ? -1 : 0);
