  ...
  const LoadMoreProducts = () => {
    ...
    if (activeApi === "storeSub" || activeApi === "categoryProd") {
      ApiService.StoreCategoryProd(apiPayload)
        .then((res) => {
          const existingProducts = categoryProd || []; // Use existingProducts if categoryProd is not yet defined
          const newProducts = res.payload_VerticalByProduct.filter(
            (product) => !existingProducts.some((p) => p.product_id === product.product_id)
          );
          const allProducts = [...existingProducts, ...newProducts];
          setShopCategoryProd(allProducts);
        })
        .catch((err) => {});
    } else if (activeApi === "categorySub") {
      ApiService.CategoryBySubProd(apiPayload)
        .then((res) => {
          const existingProducts = categoryProd || [];
          const newProducts = res.payload_SubCategoryByProduct.filter(
            (product) => !existingProducts.some((p) => p.product_id === product.product_id)
          );
          const allProducts = [...existingProducts, ...newProducts];
          setShopCategoryProd(allProducts);
