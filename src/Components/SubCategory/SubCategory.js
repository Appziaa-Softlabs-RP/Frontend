            ...
            {shopCategory?.length > 0 && 
                <React.Fragment>
                    <div className={`${styles.lookingContainer} ps-3 py-3 col-12 d-inline-flex align-items-stretch gap-3`}>
                        <ReactReactOwlCarousel className={`col-12 d-inline-block owl-theme`} margin={20} loop={false} dots={false} stagePadding={20} items={3}>
                            {shopCategory?.map((item, index) => {
                                return (
                                    <div key={index} className={`${catActive === item?.category_id && styles.active} d-inline-flex flex-column flex-shrink-0 gap-1`} onClick={() => getSubCategory(item?.category_id)} ref={(element) => (slideRef.current[index] = element)}>
